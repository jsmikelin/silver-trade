...
def api_live_price(request):
    """Serve the last validated silver feed generated by the daily job."""
    price_path = settings.BASE_DIR / "api" / "live-price.json"
    try:
        with price_path.open(encoding="utf-8") as handle:
            payload = json.load(handle)
    except (OSError, json.JSONDecodeError) as exc:
        logger.error("Validated silver price feed is unavailable: %s", exc)
        return JsonResponse(
            {"error": "Silver price data is temporarily unavailable"},
            status=503,
        )
